"use client"
import { getReferenceByTypeCode, InfoBlockFilter, PageFilter, Reference, ReferenceTypeCodesEnum } from "@/entities";
...
import { getPageFilterForm } from "../helpers";
...
export const PageFilterFeature = ({ opened, toggle }: Props) => {
    ...
    const form = useForm(getPageFilterForm(oldParams))
    ...
    const handleSubmit = () => {
        const resUrl = Actions.HandleFilterFormSubmit<PageFilter>(form.values, oldParams, pathname)
        router.push(resUrl);
        toggle()
    };
